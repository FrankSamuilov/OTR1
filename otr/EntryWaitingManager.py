...
class EntryWaitingManager:
    ...
    def _price_monitor_loop(self):
        """价格监控循环，检查是否达到入场条件，包括高级SMC入场分析和流动性事件检测"""
                                    ...
                                    for event in liquidity_events["detected_events"]:
                                        ...
                                        if event_strength >= 8:
                                            ...
                                            if event_type == "liquidity_absorption":
                                                ...
                                                direction_matched = (side == "BUY" and direction == "low") or (
                                                            side == "SELL" and direction == "high")
                                            elif event_type == "stop_hunt":
                                                direction = event.get("direction", "")
                                                direction_matched = (side == "BUY" and direction == "down_up") or (
                                                            side == "SELL" and direction == "up_down")

                                            if direction_matched:
                                                entry_conditions_met = True
                                                entry_reason = f"检测到高质量方向匹配的{event_type}流动性事件"
                                                print_colored(f"⭐ 高质量流动性事件触发立即入场条件!",
                                                              Colors.GREEN + Colors.BOLD)
                            except Exception as le:
                                print_colored(f"等待入场中检测流动性事件失败: {le}", Colors.WARNING)
    ...
    def _analyze_entry_conditions(self, symbol, side, current_price, entry_info):
        ...
        try:
            ...
            # 检测流动性事件
            from liquidity_events import detect_liquidity_events
            print_colored(f"检测 {symbol} 的等待入场流动性事件...", Colors.BLUE)
            liquidity_events = detect_liquidity_events(df, current_price)

            # 执行增强的价格位置分析
            # 这里可以调用 _analyze_price_position 方法
            print_colored("使用增强的价格位置分析，整合流动性事件", Colors.INFO)
            ...
            if position_score >= 7.0:
                ...
                should_enter = True
                reason = f"价格位置条件良好 (评分: {position_score:.2f}/10)"
            else:
                reason = f"价格位置条件较差 (评分: {position_score:.2f}/10)"
            ...
            if liquidity_events["detected_events"]:
                ...
                if liquidity_score > 6:
                    position_score = min(10, position_score + (liquidity_score - 5) * 0.4)
                    print_colored(f"流动性事件提升价格位置评分: {position_score:.2f}", Colors.GREEN)

                    if position_score >= 7.0 and not should_enter:
                        should_enter = True
                        reason = f"流动性事件改善了入场条件 (评分: {position_score:.2f}/10)"
                ...
                for event in liquidity_events["detected_events"]:
                    ...
                    if event_strength >= 8:  # 高质量流动性事件
                        if event_type == "liquidity_absorption":
                            direction = event.get("direction", "")
                            # 方向匹配的高质量流动性吸收
                            if (side == "BUY" and direction == "low") or (side == "SELL" and direction == "high"):
                                should_enter = True
                                reason = f"检测到高质量{side}方向流动性吸收"
                                print_colored(f"⭐ 高质量流动性吸收建议入场", Colors.GREEN + Colors.BOLD)

                        elif event_type == "stop_hunt":
                            direction = event.get("direction", "")
                            # 方向匹配的止损猎杀
                            if (side == "BUY" and direction == "down_up") or (
                                    side == "SELL" and direction == "up_down"):
                                should_enter = True
                                reason = f"检测到{'下方' if direction == 'down_up' else '上方'}止损猎杀后反转"
                                print_colored(f"⭐ 止损猎杀反转建议入场", Colors.GREEN + Colors.BOLD)
