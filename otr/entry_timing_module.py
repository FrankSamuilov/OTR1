...
from liquidity_events import detect_liquidity_events
...
def calculate_entry_timing(df: pd.DataFrame, signal: str,
    ...
    try:
        # 检测流动性事件
        print_colored("🔍 检测流动性事件...", Colors.BLUE)
        ...
        if liquidity_events["detected_events"]:
            print_colored("🔍 整合流动性事件到入场计划...", Colors.GREEN)

            # 检查是否存在高质量流动性事件
            if liquidity_events["quality_score"] >= 7.0:
                liquidity_entry = get_liquidity_based_entry(liquidity_events, signal, current_price)

                if liquidity_entry:
                    # 高质量流动性事件可能覆盖之前的入场策略
                    result["expected_entry_price"] = liquidity_entry["price"]
                    result["entry_type"] = liquidity_entry["type"]
                    result["should_wait"] = liquidity_entry["should_wait"]
                    result["immediate_entry"] = not liquidity_entry["should_wait"]
                    result["confidence"] += 0.2
                    entry_conditions.append(liquidity_entry["description"])

                    # 如果是极高质量事件，可能建议立即入场
                    if liquidity_events["quality_score"] >= 9.0 and abs(
                            current_price - liquidity_entry["price"]) / current_price < 0.005:
                        result["should_wait"] = False
                        result["immediate_entry"] = True
                        result["entry_type"] = "MARKET"
                        entry_conditions.append("极高质量流动性事件确认，建议立即入场")

            # 添加检测到的流动性事件作为入场条件
            for event in liquidity_events["detected_events"]:
                if event["type"] == "liquidity_absorption":
                    direction = "上方" if event.get("direction") == "high" else "下方"
                    entry_conditions.append(f"检测到{direction}流动性吸收，强度: {event['strength']:.1f}")
                elif event["type"] == "stop_hunt":
                    direction = "向下突破后反转" if event.get("direction") == "down_up" else "向上突破后反转"
                    entry_conditions.append(f"检测到止损猎杀，{direction}，强度: {event['strength']:.1f}")
                elif event["type"] == "imbalance":
                    if "zone" in event:
                        zone = event["zone"]
                        zone_type = "买方" if zone["type"] == "bullish" else "卖方"
                        entry_conditions.append(f"检测到{zone_type}价格不平衡区域，强度: {event.get('strength', 5):.1f}")
                elif event["type"] == "institutional_intent":
                    entry_conditions.append(f"检测到机构交易意图，强度: {event.get('strength', 5):.1f}")
...
def get_liquidity_based_entry(liquidity_events, signal, current_price):
    """
    根据流动性事件计算最佳入场策略
    ...
    返回:
        dict: 基于流动性事件的入场策略
    """
    if not liquidity_events["detected_events"]:
        return None
    ...
    if liquidity_events["liquidity_absorption"]["detected"]:
        ...
        if (signal == "BUY" and la_direction == "low") or (signal == "SELL" and la_direction == "high"):
            # 方向匹配的流动性吸收
            entry_price = la_price * (1.005 if signal == "BUY" else 0.995)
            quality = la["strength"]
            ...
            if quality > highest_quality:
                ...
                should_wait = price_diff_pct > 0.3  # 超过0.3%差异则等待

                best_entry = {
                    "price": entry_price,
                    "type": "LIMIT" if should_wait else "MARKET",
                    "should_wait": should_wait,
                    "source": "liquidity_absorption",
                    "description": f"基于{'低点' if la_direction == 'low' else '高点'}流动性吸收{'等待' if should_wait else '入场'}"
                }
    ...
    if liquidity_events["stop_hunt"]["detected"]:
        ...
        if (signal == "BUY" and sh_direction == "down_up") or (signal == "SELL" and sh_direction == "up_down"):
            # 方向匹配的止损猎杀
            entry_price = sh_level * (0.997 if signal == "BUY" else 1.003)
            quality = sh["strength"]
            ...
            if quality > highest_quality:
                ...
                best_entry = {
                    ...
                    "should_wait": should_wait,
                    "source": "stop_hunt",
                    "description": f"基于止损猎杀{'等待' if should_wait else '入场'}，类型: {sh_direction}"
                }
    ...
    if liquidity_events["imbalance"]["detected"] and liquidity_events["imbalance"]["recent_zone"]:
        ...
        if not imb_zone["filled"]:
            zone_type = imb_zone["type"]

            if (signal == "BUY" and zone_type == "bullish") or (signal == "SELL" and zone_type == "bearish"):
                # 方向匹配的不平衡区域
                entry_price = imb_zone["start_price"] if zone_type == "bullish" else imb_zone["end_price"]
                quality = 8.0  # 不平衡区域通常是高质量入场点
                ...
                if quality > highest_quality:
                    ...
                    best_entry = {
                        ...
                        "should_wait": should_wait,
                        "source": "imbalance",
                        "description": f"基于{zone_type}不平衡区域{'等待' if should_wait else '入场'}"
                    }
    ...
    if liquidity_events["institutional_intent"]["detected"]:
        ...
        for ev in intent["evidence"]:
            if ev["type"] in ["multiple_tests_of_support", "multiple_tests_of_resistance"]:
                ...
                if (signal == "BUY" and ev["type"] == "multiple_tests_of_support") or \
                        (signal == "SELL" and ev["type"] == "multiple_tests_of_resistance"):
                    # 方向匹配的机构测试水平
                    entry_price = level * (1.003 if signal == "BUY" else 0.997)
                    quality = ev.get("test_count", 3) * 1.5
                    ...
                    if quality > highest_quality:
                        ...
                        best_entry = {
                            ...
                            "type": "LIMIT" if should_wait else "MARKET",
                            "should_wait": should_wait,
                            "source": "institutional_intent",
                            "description": f"基于机构多次测试{'支撑' if ev['type'] == 'multiple_tests_of_support' else '阻力'}位{'等待' if should_wait else '入场'}"
                        }
    ...
    try:
        ...
        try:
            ...
            if liquidity_events["liquidity_absorption"]["detected"]:
                ...
                if la_direction == "high" and current_price > la_level:
                    # 上方流动性吸收后价格继续上涨 - 强信号
                    la_strength = (current_price - la_level) / la_level * 100 * 1.5  # 加强权重
                    breakout_details.append({
                        "type": "liquidity_absorption",
                        "direction": "UP",
                        "description": f"吸收上方流动性后继续上涨（{la_level:.6f}）",
                        "strength": la_strength
                    })
                elif la_direction == "low" and current_price < la_level:
                    # 下方流动性吸收后价格继续下跌 - 强信号
                    la_strength = (la_level - current_price) / la_level * 100 * 1.5  # 加强权重
                    breakout_details.append({
                        "type": "liquidity_absorption",
                        "direction": "DOWN",
                        "description": f"吸收下方流动性后继续下跌（{la_level:.6f}）",
                        "strength": la_strength
                    })
            ...
            if liquidity_events["stop_hunt"]["detected"]:
                ...
                # 止损猎杀后价格强烈反转，形成突破
                if sh_direction == "down_up" and current_price > (sh_level * 1.01):  # 向上反转1%以上
                    sh_strength = (current_price - sh_level) / sh_level * 100 * 1.3  # 加强权重
                    breakout_details.append({
                        "type": "stop_hunt_breakout",
                        "direction": "UP",
                        "description": f"止损猎杀后向上突破（{sh_level:.6f}）",
                        "strength": sh_strength
                    })
                elif sh_direction == "up_down" and current_price < (sh_level * 0.99):  # 向下反转1%以上
                    sh_strength = (sh_level - current_price) / sh_level * 100 * 1.3  # 加强权重
                    breakout_details.append({
                        "type": "stop_hunt_breakout",
                        "direction": "DOWN",
                        "description": f"止损猎杀后向下突破（{sh_level:.6f}）",
                        "strength": sh_strength
                    })
        except Exception as le:
            print_colored(f"检测流动性相关突破时出错: {le}", Colors.WARNING)
            # 继续处理其他突破类型
